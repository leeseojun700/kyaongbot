...
function response(room, msg, sender, isGroupChat, replier, ImageDB) {
    ...
    try {
        ...
        if (msg == "!캬옹봇") {
            replier.reply('캬옹봇 - 자바스크립트 기반 카카오톡 자동응답 매크로 스크립트(이하 캬옹봇이라 지칭)의 제작자는 "불여우"이며, 캬옹봇 소스 코드에는 GPL 라이선스가 적용되어 있고 이를 준수할 시 자유롭게 사용 및 수정할 수 있습니다.코드의 일부분 또는 전체를 사용시 본 소스 코드를 포함하여야 하고(원본 배포 전염성) 공개 배포해야 하며(파생물 배포 전염성), 똑같은 라이선스를 적용시켜야 합니다.')
        }
        if (msg == "!차트") {//!차트면
            var charts = [];
            for (var i = 1; i < 51; i++) {
