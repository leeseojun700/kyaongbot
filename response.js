...
function response(room, msg, sender, isGroupChat, replier, ImageDB) {
    ...
    if (count[room] == 20) { //200번 다 채우면,
        replier.reply("방 옮김:https://open.kakao.com/o/gvr3ikL");
  count[room] = 0; //0으로 초기화
    }}    
if(admin.indexOf(sender)>-1) {	
    ...
    if (msg == "/on") {
        botOn = true;
        Bot.say("ea가 활성화되었습니다.", replier);
    }
    if (msg == "/off") {
        botOn = false;
        Bot.say("ea가 비활성화되었습니다.", replier);
    }
	if(msg.trim()=="!카운트"){
   ...
   replier.reply(wiki(msg.substring(4)))
    ...
    for(var n=0;n<words.length;n++){
        if(msg.indexOf(words[n])!=-1){
            war = parseInt(DB.readData(sender))
            if (war == null) { //이미 저장된게 없다면
                DB.saveData(sender, "1"); //새로 저장
                replier.reply("쓰읍! 방금 욕설이 감지 되었습니다! 욕설 사용에 주의 하십시오.\n"+sender+"님의 경고수는 1회 입니다\n 10회 이상시 강퇴될 수 있습니다.");
            } else { //이미 저장된게 있다면,
                DB.saveData(sender, war+1);
                replier.reply("쓰읍! 방금 욕설이 감지 되었습니다! 욕설 사용에 주의 하십시오.\n"+sender+"님의 경고수는"+war+"/10 입니다\n 10회 이상시 강퇴될 수 있습니다.");
            }
        break;
        }
        }
