  ...
  while (true) {
    ...
    if (DB.icode.indexOf(n) == -1) {
      ...
      for (var i = 0; i < DB.icode.length; i++) {
           ta2.push(DB.icode[DB.inick.indexOf(ta[i])])
           DB.inick.sort();
           replier.reply(i)
           replier.reply(DB.icode[ta.indexOf(DB.inick[i])])
       }
       replier.reply(ta2)
      DB.icode = ta2
...
if (msg.split(" ")[0] == "!호출") {
    ...
    var o = scode
    var s = msg.split(" ")[1]
    if (DB.p[scode].call[s].indexOf(o) != -1) {
        replier.reply("이미 상대를 호출했습니다.")
    }
    var m = msg.substr(9)
    DB.p[scode].call[s].push(o)
    DB.p[scode].callmsg[s].push(escape(m))
