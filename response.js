...
// 배열 백업 기능!
var loadArray = DataBase.getDataBase(loadArray).split('~!+!~'); //배열이름의 배열 덩어리를 쪼개서 다시 배열로 만들기
for (var i = 0; loadArray.length > i; i++) { //배열이름마다 배열덩어리를 배열로 쪼개기 반복
  eval(loadArray[i] + " = DataBase.getDataBase(loadArray[i]).split('~!+!~')"); //동적변수를 가져오는데 eval을 사용
}
      ...
      if (admin.indexOf(sender) > -1) {
        ...
        if (msg == "!리로드") {
          for (var i = 0; saveArray.length > i; i++) { //배열명 배열 안의 변수마다 반복
            DataBase.setDataBase(saveArray[i].join(separator = '~!+!~'), eval(saveArray[i])) //배열명(eval로 로드)마다 배열을 합친 덩어리를 값으로 DB에 저장
            DataBase.setDataBase(loadArray.join(separator = '~!+!~'), "loadArray")
          }
          Api.reload();
