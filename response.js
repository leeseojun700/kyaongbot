...
// DB.p.call : 객체
// DB.p.call[scode] : 배열(스트링), scode를 호출한 사람
// DB.p.callmsg : 객체
// DB.p.callmsg[scode] : 배열(스트링), scode를 호출한 사람이 남기는 메시지
...
if ("call" in DB.p == false) DB.p.call = new Object()
if ("callmsg" in DB.p == false) DB.p.callmsg = new Object()
...
if (DB.inick.indexOf(sender) == -1) {
  while (true) {
    ...
    if (DB.icode.indexOf(n) == -1) {
      ...
      DB.p.rps[scode] = 0
      DB.p.call[scode] = new Array()
      DB.p.callmsg[scode] = new Array()
      replier.reply(scode)
      break loop;
    }
  }
...
if (msg.trim() == "!카운터 공지") replier.reply(DB.ncounter + "/300")

if (msg.trim() == "!카운터 자신") replier.reply("[" + sender + "]\n" + DB.p.counter[scode])
...
if (msg.split(" ")[0] == "!상점") {
    ...
    if (DB.p.pt[scode] - i < 0) {
        var x = i - DB.p.pt[scode];
        replier.reply("포인트가 " + x + "cp 부족합니다. VAT 20%를 유의해 주세요.");
        break loop;
    }
    DB.p.pt[scode] -= i;
    DB.p.pt[msg.split(" ")[1]] += a;
...
if (msg.split(" ")[0] == "!호출") {
    ...
    var o = scode
    var s = msg.split(" ")[1]
    if (DB.p.call[s].indexOf(o) != -1) {
        replier.reply("이미 상대를 호출했습니다.")
    }
    var m = msg.substr(9)
    DB.p.call[s].push(o)
    DB.p.callmsg[s].push(escape(m))
